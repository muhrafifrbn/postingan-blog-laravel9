    ...
    <div class="col-lg-8">
        <form method="POST" action="/dashboard/posts/{{ $post->slug }}" enctype="multipart/form-data" class="mb-5">
            ...
            <div class="mb-3">
              ...
              <input autofocus required name="title" value="{{ old("title", $post->title) }}" type="text" class="form-control @error('title') is-invalid @enderror" id="title" aria-describedby="emailHelp">
              @error('title')
            ...
            <div class="mb-3">
              ...
              <input name="slug" required type="text" value="{{ old("slug", $post->slug) }}" class="form-control @error('slug') is-invalid @enderror" id="slug" aria-describedby="emailHelp">
              @error('slug')
              <div class="invalid-feedback">
                {{ $message }}
              </div>
            ...
            <div class="mb-3">
              ...
              @if ($post->image)
              <img src="{{ asset("storage/".$post->image) }}" class="d-block img-preview img-fluid col-sm-6 mb-3" alt="">
              @else
